                        ...
                        <div class="card-body collapse in">
                            <div class="table-responsive card-block">
                                    ...
                                    <div class="col-xs-12 col-md-center">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="btn-group mb-0.1 ml-0.1 " style="float:lefth; margin-top:-15px;">
                                                <a href="{{ route('pemasukan.create')}}" class="btn btn-info btn-outline-primary mb-0.1 ">Tambah Data</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <table class="table table-bordered mb-4 ">
                                    ...
                                    <tr>
                                        <td>{{ ++$i }}</td>
                                        <td>{{ $pemasukan1->rincian_transaksi }}</td>
                                        <td>Rp. {{ number_format($pemasukan1->jumlah,0,',','.') }}</td>
                                        <td>{{ $pemasukan1->created_at}}</td>
                                        <td>
                                            <!-- Button trigger modal -->
                                            <a href="#del{{$pemasukan1->id_transaksi}}" class="btn btn-danger" data-toggle="modal">Hapus </a>


                                            <a href="{{ route('pemasukan.show', $pemasukan1->id_transaksi) }}" class=" btn btn-primary">
                                            Lihat
                                            </a>
                                            <a href="{{ route('pemasukan.edit', $pemasukan1->id_transaksi) }}" class=" btn btn-warning">
                                            Edit
                                            </a>
                                        <!-- Modal -->
                                            <div class="modal fade bd-example-modal-sm" id="del{{$pemasukan1->id_transaksi}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                <div class="modal-dialog modal-sm" role="document">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body" style="text-align: center;">
                                                            Apakah Anda ingin Menghapus Data <b>{{$pemasukan1->id_transaksi}}</b>
                                                        </div>
                                                        <div class="modal-footer">
                                                            <!-- <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button> -->
                                                            <form action="{{ route('pemasukan.destroy', $pemasukan1->id_transaksi) }}" method="POST">
                                                            @csrf
                                                            @method('DELETE')
